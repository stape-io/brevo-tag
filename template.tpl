...

{
  ...
  "description": "Sends events to Brevo.",
  "containerContexts": [
    "SERVER"
  ]
}
...
[
  {
    ...
    "subParams": [
      ...
      },
      {
        ...
        "help": "How to obtain the Client Key:\n\u003cul\u003e\n\u003cli\u003e\u003ca href\u003d\"https://developers.brevo.com/docs/tracker-rest-implementation#authentication\"\u003ev2\u003c/a\u003e\u003c/li\u003e\n\u003cli\u003e\u003ca href\u003d\"https://developers.brevo.com/docs/api-key-authentication\"\u003ev3\u003c/a\u003e\u003c/li\u003e\n\u003c/ul\u003e"
      },
      {
        "type": "TEXT",
        "name": "email",
        ...
        "help": "Email of the user. \n\u003cbr\u003e\u003cbr\u003e \n\u003cb\u003ev2\u003c/b\u003e: required. \n\u003cbr\u003e\n\u003cb\u003ev3\u003c/b\u003e: optional, but at least one user identifier must be sent. Check \u003ci\u003eCustomer Identifiers\u003c/i\u003e section below."
      },
      {
        "type": "TEXT",
        "name": "event",
      ...
      },
      {
        ...
        "help": "The Link that was clicked."
      },
      {
        "type": "TEXT",
        "name": "page",
        ...
        "help": "The Page Name where the event happened."
      },
      {
        "type": "CHECKBOX",
        "name": "storeEmail",
        ...
        "simpleValueType": true,
        "help": "Enable email persistence via cookies to serve as a fallback for events where no email is specified."
      },
      {
        "type": "CHECKBOX",
...
scenarios:
- name: '[V2] URL, headers and payload are correctly built for each event type'
  code: |-
    const expectedValue = 'test';
    const inputTableMap = [{ name: expectedValue, value: expectedValue }];
    const expectedMap = makeTableMap(inputTableMap, 'name', 'value');

    [
      {
        type: 'trackPage',
        extraMockData: { page: expectedValue, properties: inputTableMap },
        expectedBody: { properties: expectedMap, email: expectedValue, page: expectedValue }
      },
      {
        type: 'trackEvent',
        extraMockData: { event: expectedValue, properties: inputTableMap, propertiesEvent: inputTableMap },
        expectedBody: { properties: expectedMap, eventData: expectedMap, email: expectedValue, event: expectedValue }
      },
      {
        type: 'trackLink',
        extraMockData: { link: expectedValue, properties: inputTableMap },
        expectedBody: { properties: expectedMap, email: expectedValue, link: expectedValue }
      },
      {
        type: 'identify',
        extraMockData: { customerProperties: inputTableMap },
        expectedBody: { attributes: expectedMap, email: expectedValue }
      }
    ].forEach((scenario) => {
      mockData.apiVersion = 'v2';
      mockData.type = scenario.type;
      mockData.email = expectedValue;

      for (const key in scenario.extraMockData) {
        mockData[key] = scenario.extraMockData[key];
      }

      mock('sendHttpRequest', (url, callback, options, body) => {
        const bodyParsed = JSON.parse(body);
        assertThat(url).isEqualTo('https://in-automate.brevo.com/api/v2/' + scenario.type);
        ...
        assertThat(options.headers['Accept']).isEqualTo('application/json');

        for (const bodyKey in scenario.expectedBody) {
          assertThat(bodyParsed[bodyKey]).isEqualTo(scenario.expectedBody[bodyKey]);
        }
...
- name: '[V3] URL, headers and payload are correctly built for each event type'
  code: |-
    const expectedValue = 'test';
    const inputTableMap = [{ name: expectedValue, value: expectedValue }];
    const expectedMap = makeTableMap(inputTableMap, 'name', 'value');
    const inputIdentifiers = [{ name: 'ext_id', value: expectedValue }, { name: 'phone_id', value: expectedValue }];
    const expectedIdentifiers = makeTableMap(inputIdentifiers, 'name', 'value');
    expectedIdentifiers.email_id = expectedValue;

    [
      {
        type: 'trackPage',
        extraMockData: { page: expectedValue, properties: inputTableMap },
        expectedBody: { event_name: 'page_view', page: expectedValue, event_properties: expectedMap, identifiers: expectedIdentifiers }
      },
      {
        type: 'trackEvent',
        extraMockData: { event: expectedValue, properties: inputTableMap, propertiesEvent: inputTableMap },
        expectedBody: { event_name: expectedValue, contact_properties: expectedMap, event_properties: expectedMap, identifiers: expectedIdentifiers }
      },
      {
        type: 'trackLink',
        extraMockData: { link: expectedValue, properties: inputTableMap },
        expectedBody: { event_name: 'link', link: expectedValue, event_properties: expectedMap, identifiers: expectedIdentifiers }
      },
      {
        type: 'identify',
        extraMockData: { customerProperties: inputTableMap },
        expectedBody: { event_name: 'identify', contact_properties: expectedMap, identifiers: expectedIdentifiers }
      }
    ].forEach((scenario) => {
      mockData.apiVersion = 'v3';
      mockData.type = scenario.type;
      mockData.email = expectedValue;
      mockData.customerIdentifiers = inputIdentifiers;
...
- name: '[Cookie] Saves email in cookie when storeEmail is enabled and email is provided'
  code: |-
    const expectedValue = 'test';

    [
      { apiVersion: 'v2', type: 'trackPage', page: expectedValue },
      { apiVersion: 'v3', type: 'trackPage', page: expectedValue }
    ].forEach((scenario) => {
      mockData.apiVersion = scenario.apiVersion;
      mockData.type = scenario.type;
      mockData.page = scenario.page;
...
- name: '[Cookie] Uses email from cookie when storeEmail is enabled and email is not
    provided'
  code: |-
    const expectedValue = 'test';
...
- name: '[Required Fields] Request is not sent when required identifiers are missing'
  code: |-
    [
      { apiVersion: 'v2', description: 'V2 without email' },
      { apiVersion: 'v3', description: 'V3 without any identifier' }
    ].forEach((scenario) => {
      mockData.apiVersion = scenario.apiVersion;
...
- name: '[Early Exit] Consent denied via consent_state stops execution'
  code: |
    ...
    assertApi('gtmOnSuccess').wasCalled();
    assertApi('gtmOnFailure').wasNotCalled();
- name: '[Early Exit] Consent granted via consent_state proceeds normally'
  code: |-
    mockData.adStorageConsent = 'required';
    mockData.apiVersion = 'v2';
    mockData.type = 'trackPage';
    ...
    assertApi('gtmOnSuccess').wasCalled();
    assertApi('gtmOnFailure').wasNotCalled();
- name: '[Early Exit] Consent check via x-ga-gcs grants and denies correctly'
  code: |
    [
      {
        gcsValue: 'G110',
        shouldSend: true,
        description: 'granted (3rd char is 1)'
      },
      { gcsValue: 'G100', shouldSend: false, description: 'denied (3rd char is 0)' }
    ].forEach((scenario) => {
      mockData.adStorageConsent = 'required';
      mockData.apiVersion = 'v2';
      mockData.type = 'trackPage';
      ...
      mock('getAllEventData', () => {
        return {
          event_name: 'page_view',
          page_location: 'https://example.com',
          'x-ga-gcs': scenario.gcsValue
        };
      });

      let sendHttpRequestCalled = false;
      mock('sendHttpRequest', (url, callback, options, body) => {
        if (!scenario.shouldSend) {
          fail('sendHttpRequest should not be called when consent is denied');
        }
      ...
      assertApi('gtmOnFailure').wasNotCalled();
      if (scenario.shouldSend) {
        assertThat(sendHttpRequestCalled).isTrue();
      } else {
        assertThat(sendHttpRequestCalled).isFalse();
      }
    });
...
- name: '[Early Exit] Calls gtmOnSuccess and stops when URL matches GTM preview endpoint'
  code: |-
    ...
    assertApi('gtmOnFailure').wasNotCalled();
    assertApi('sendHttpRequest').wasNotCalled();
- name: '[Default Type] Calls gtmOnFailure for unknown event type'
  code: |-
    mockData.apiVersion = 'v2';
    mockData.type = 'unknownType';
    mockData.email = 'test@example.com';
    ...
    assertApi('gtmOnFailure').wasCalled();
    assertApi('sendHttpRequest').wasNotCalled();
- name: '[API Version] Defaults to v2 when apiVersion is not specified'
  code: |-
    mockData.apiVersion = undefined;
    mockData.type = 'trackPage';
    mockData.page = 'test';
    mockData.email = 'test@example.com';

    mock('sendHttpRequest', (url, callback, options, body) => {
      assertThat(url).isEqualTo('https://in-automate.brevo.com/api/v2/trackPage');
      assertThat(options.headers['ma-key']).isEqualTo(mockData.clientKey);
    ...
    assertApi('gtmOnSuccess').wasCalled();
    assertApi('gtmOnFailure').wasNotCalled();
- name: '[Response] Calls gtmOnSuccess on 2xx response without optimistic scenario'
  code: |-
    [200, 201, 204].forEach((statusCode) => {
      ...
      mockData.page = 'test';
      mockData.email = 'test@example.com';
      mockData.useOptimisticScenario = false;
...
- name: '[Response] Calls gtmOnFailure on non-2xx response without optimistic scenario'
  code: |-
    [400, 401, 500].forEach((statusCode) => {
      mockData.apiVersion = 'v2';
      mockData.type = 'trackPage';
      mockData.page = 'test';
      mockData.email = 'test@example.com';
...
- name: '[Optimistic] Calls gtmOnSuccess immediately without waiting for response'
  code: |-
    mockData.apiVersion = 'v2';
    mockData.type = 'trackPage';
    mockData.page = 'test';
    ...
    mock('sendHttpRequest', (url, callback, options, body) => {
      // Simulate a delayed callback: the tag should NOT wait for this.
      // In optimistic mode, gtmOnSuccess is called before the callback fires.
    });
...
setup: |-
  ...
  // Default: container is NOT in debug/preview mode
  ...
  // Default: sendHttpRequest resolves with 200
  mock('sendHttpRequest', (url, callback, options, body) => {
    const response = { statusCode: 200, headers: {}, body: '' };
    return callback(response.statusCode, response.headers, response.body);
  });
